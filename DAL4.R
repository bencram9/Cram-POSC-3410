...
# View the code in a separate window
gss_df %>% 
  view()

AnswerA <- "The columns represent questions asked on the GSS, and the rows represent the respondents"
...
# Find the missing observations in the count
gss_df %>% 
  group_by (YEAR) %>% 
  count() %>% 
  filter(is.na(YEAR))
...
Answer1_expectation <- "We have a bar graph of two variables. Sex should be on the x axis and Number should be on y. About a 50-50% split."
...
Answer1 <- "There are more females than males, more than the expected 50-50%"
...
Answer2_expectation <- "Bar Graph. Race on x and Number on Y. White, Black, and Other. At least plurality white (more than 30%), around 15%, I don't know."
...
# Use a text vector to explain what is peculiar about the result that you see. 
Answer2 <- "There is a very high percentage of White, Black appears to be below 15%, Other is a small percentage."
...
Question3_part1 <- "The values are decimals."
...
Question3_part2 <- "Weight variable. I think it might be the sample weight of the survey. "
