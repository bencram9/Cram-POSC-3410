...
# Put engine size on y-axis, and color by class.
mpg_plot<-mpg_data + geom_point(mapping = aes(x=displ , y=hwy, color=class))
# Call mpg_plot
mpg_plot
# What do you see?
# a very nice looking plot
# Now, let's add an aesthetic layer to change the axis labels
mpg_axis <- mpg_plot + xlab("Engine Size in Liters") + ylab("Highway Milage") + ggtitle("Engine Size and Mileage by Vehicle Class")
# Call mpg_axis
mpg_axis
# What do you see?
# Changed axis labels
...
flights_df<-filter(flights, (!is.na(year)& !is.na(month)& !is.na(dep_delay)& !is.na(arr.delay)& !is.na(carrier)& !is.na(flight)& !is.na(air_time)))
# Call flights_df
flights_df
...
select(flights, dep_delay, carrier, flight, origin, dest, air_time, distance, hour)
...
summarise(not_cancelled, mean(dep_delay) & mean(arr_delay))
...
flights %>%
  ...
  head(origin, dest, arr_delay, dep_delay, carrier)%>%
  ggplot(x=dep_delay, y=arr_delay, color=carrier)%>%
# To which airports did the most flights from NYC go? Make a ggplot visualization.
flights %>%
  filter(origin==NYC) %>%
  ggplot(x=origin, y=dest) %>%
